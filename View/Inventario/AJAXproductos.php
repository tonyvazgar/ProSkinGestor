<?php
    ...
    if($marca != "AINHOA"){     //No liene lineas de productos aparte
      ...
      if(empty($productos) && !isset($_POST['linea'])){
      ...
      foreach($productos as $d){
        if($d['stock_disponible_producto'] == 0){
          echo "<li class='list-group-item d-flex justify-content-between align-items-center'>
                  <a href='detallesProducto.php?id=".$d['id_producto']."' role='button'>".$d['descripcion_producto']." - (".$d['presentacion_producto'].") - $".number_format($d['costo_unitario_producto'])."</a><span class='badge bg-warning rounded-pill'>Sin Stock</span>
                </li>";
        }else{
          echo "<li class='list-group-item d-flex justify-content-between align-items-center'>
                  <a href='detallesProducto.php?id=".$d['id_producto']."' role='button'>
                  ".$d['descripcion_producto']." - (".$d['presentacion_producto'].") - $".number_format($d['costo_unitario_producto'])."
                  </a>
                  <div>
    ...
    if(isset($_POST['linea'])){
      ...
      foreach($productos as $d){
        if($d['stock_disponible_producto'] == 0){
          ...
          echo "<li class='list-group-item d-flex justify-content-between align-items-center'>
                  <a href='detallesProducto.php?id=".$d['id_producto']."' role='button'>".$d['descripcion_producto']." - (".$d['presentacion_producto'].") - $".number_format($d['costo_unitario_producto'])."</a>
                  <div>
                    <span class='badge bg-success rounded-pill'>Stock</span>
                    <a class='btn btn-success' href='ventaProducto.php?id=".$d['id_producto']."' role='button'>Vender</a>
                  </div>
